...
def end_session(session_id):
    # Fetch activities for the current session
    url = f'http://localhost:3000/api/sessions/{session_id}'
    headers = {'accept': 'application/json'}
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
        session_data = response.json()
        total_score = sum(activity['score'] for activity in session_data['activities']) if session_data['activities'] else 0
        # End the session with the total score
        end_url = 'http://localhost:3000/api/sessions'
        end_data = json.dumps({'session_id': session_id, 'score': total_score})
        end_response = requests.put(end_url, headers={'accept': 'application/json', 'Content-Type': 'application/json'}, data=end_data)
        ...
        st.error('Failed to fetch session activities')

# Initialize session state for UI control
if 'session_data' not in st.session_state:
    st.session_state.session_data = None
    
session_id = st.session_state.session_data['id'] if st.session_state.session_data else None
if st.button("End Session" if session_id else "Start Session"):
    if session_id:
...
st.title('Hindi Writing Practice')

if st.session_state.session_data is not None:
    # Load the rest of the UI only if the session is active
    ...
    if st.button('Generate Word'):
        if 'session_data' in st.session_state:
            # Reset the uploaded image and any previous challenge state
            if 'uploaded_image' in st.session_state:
                del st.session_state.uploaded_image
            if 'fetched_word' in st.session_state:
                del st.session_state.fetched_word
            ...
            if random_word_response.status_code == 200:
                ...
                st.error('Failed to fetch new word')
        else:
